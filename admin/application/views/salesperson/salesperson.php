...
<div class="clearfix"></div>
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
     ...
     <div class="row pt-2 pb-2">
        <div class="col-sm-9">
		    <h4 class="page-title">Sales Person</h4>
    ...
    <!-- End Breadcrumb-->
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header"><span> Sales Person Details</span> 
             <a href="<?php echo BEGIN_PATH ?>/salesperson/salespersonadd" class="btn btn-success btn-sm waves-effect waves-light m-1" style="float:right;">ADD</a>
            </div>
              ...
              <table id="default-datatable" class="table table-bordered">
                <thead style="background-color: #88a8b1;color: white;">
                    <tr>
                        ...
                        <th>Customer List</th> 
                        <th>Customer's Total Credit/Debit details</th>
                        <th>Status</th>
                ...
                <tbody>
                  ...
                  ?>
                    <tr>
                        ...
                        <td><?=$value['salesperson_name']?> <?=$value['salesperson_lastname']?></td>
                        <td><?=$value['salesperson_mobile']?></td>
                        <td><?=$value['salesperson_email']?></td> 
                        
                       <td>
                          <a href="<?php echo BEGIN_PATH ?>/customer?id=<?=$value['id']?>">Customer List</a> </td>
                          
                          <td>
                          <a href="<?php echo BEGIN_PATH ?>/salesperson/totalamounts?id=<?=$value['id']?>">total credit/debit amount details</a> </td>
                                               <td>
                            <input type="hidden" name="uid" id="uid<?=$i?>"  value="<?=$value['id']?>" />
                             <input type="hidden" name="tval" id="tval"  value="" />
                            <select class="form-control" name="status" id="status<?=$i?>" style="width:100px;"
  onChange="return get(<?=$i?>);">
                                <option value="<?php if($value['status']==""){ echo 'selected';} ?>">Selected</option>
                                <option <?php if($value['status']=="Enable"){ echo 'selected';} ?>  value="Enable">Active</option>
                                <option <?php if($value['status']=="Disable"){ echo 'selected';} ?>  value="Disable">InActive</option>
                            </select>
                        </td>

                        
                        <td>
                          <a href="<?php echo BEGIN_PATH ?>/salesperson/salespersonedit?id=<?=$value['id']?>"><button class="btn btn-info waves-effect waves-light m-1"><i class="fa fa-edit"></i>
                          </button></a>
                          <a href="<?php echo BEGIN_PATH ?>/salesperson/salespersonview?id=<?=$value['id']?>"><button class="btn btn-warning waves-effect waves-light m-1">
                            <i class="fa fa-eye" aria-hidden="true"></i></button></a>
                          <a href="<?php echo BEGIN_PATH?>/salesperson/del?did=<?=$value['id']?>" 
  ...
  window.location.href="<?php echo BEGIN_PATH?>/salesperson/update?uid="+uid+"&status="+status;
  return true;
  }
  </script>
